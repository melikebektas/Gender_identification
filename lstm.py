...
headers = pad_sequences(sequences,maxlen=max_len)
...
for train, test in skf.split(headers, label_list):
    ...
    history = model.fit(headers[train],label_list[train],
                        validation_data = (headers[test], label_list[test]),
...
print("LSTM Train std Accuracy: ", np.std(accList))
print("LSTM Test std Accuracy: ",np.std(accListVal))
print("LSTM Train std Precision: ",np.std(precision_value_train))
print("LSTM Test std Precision: ",np.std(precision_value_test))
print("LSTM Train std Recall : ",np.std(recall_value_train))
print("LSTM Test std Recall : ",np.std(recall_value_test))
...
a=0
for a in range(len(epochDizisi)):
  print("Accuracy: ", epochDizisi[a])
  #print("Error: ", np.std(accuracies_train2))
  print("Accuracy: ", np.std(epochDizisi[a]))
  a=a+1
